...
const char DEFAULT_WALLET_FILE_NAME[] = "Constella.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.constellacoin.cl:1313";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("Constellawallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("Constellawallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("Constellawallet.desktop");
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.Constella.Constellawallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Constella Wallet\n");
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\Constella", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:Constella");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\Constella\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\Constella\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("Constellawallet.cfg"));
